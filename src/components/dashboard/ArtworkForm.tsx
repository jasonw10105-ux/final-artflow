...
type ArtworkStatus = AppArtwork['status'];
type Condition = AppArtwork['condition'];
type Rarity = AppArtwork['rarity'];
type FramingStatus = AppArtwork['framing_status'];
...
interface ArtworkFormProps {
  ...
  // New prop to handle changing the active artwork from the sidebar
  onArtworkSelectedFromSidebar?: (artworkId: string) => void;
}
...
// Helper to get primary image URL from an array of images (or first available)
const getPrimaryImageUrl = (artworkImages: ArtworkImage[] | null | undefined): string | null => {
    if (!artworkImages || artworkImages.length === 0) return null;
    const primary = artworkImages.find(img => img.is_primary);
    // Corrected the optional chaining here
    return primary?.image_url || artworkImages[0]?.image_url || null;
};

// Assuming this RPC exists or a direct DB update will be used for editions
...
// ---------------------------------------------------
// 3. Custom Hook: useEditionManagement
// ---------------------------------------------------
...
const BENCH_REAL_WIDTH_M = 1.5;
const BENCH_PIXEL_WIDTH = 500;
const ARTWORK_TARGET_WALL_POS = { x: 0.5, y: 0.4 };
...
const extractDominantColor = async (imageUrl: string): Promise<string[] | null> => {
  try {
    ...
    // Ensure all three RGB components are passed to toHex
...
async function generateWatermark(
  ...
  try {
    ...
    const fontSize = Math.max(16, Math.min(canvas.width / 25, canvas.height / 25));
    ctx.font = `${fontSize}px WatermarkFont, Arial, sans-serif`;
...
// ---------------------------------------------------
// 5. Reusable SortableItem component for Historical Entries
// ---------------------------------------------------
...
// ---------------------------------------------------
const ArtworkForm: React.FC<ArtworkFormProps> = ({ artworkId, formId, onSaveSuccess, onTitleChange, onArtworkSelectedFromSidebar }) => {
  ...
  const [artistFullName, setArtistFullName] = useState<string>('');
  ...
  const [isReplacingExistingImage, setIsReplacingExistingImage] = useState<string | null>(null);
  const [isProcessingImagesClientSide, setIsProcessingImagesClientSide] = useState(false);
  ...
  const triggerMetadataUpdateBackend = useCallback(async (
    ...
    payload: {
      ...
      keywords_from_image?: string[] | null;
    } = {}
  ) => {
    try {
    ...
    try {
      ...
      if (force || !currentArtworkData.dominant_colors || currentArtworkData.dominant_colors.length === 0) {
        ...
        if (dominantColors && JSON.stringify(dominantColors) !== JSON.stringify(currentArtworkData.dominant_colors)) {
          latestDominantColors = dominantColors;
      ...
      const needsWatermark = force || !currentPrimaryImage.watermarked_image_url ||
                             ...
                             (currentArtworkData.title !== artwork.title) || (currentArtistFullName !== artistFullName);
      ...
      if (shouldTriggerBackendMetadataUpdate || JSON.stringify(latestDominantColors) !== JSON.stringify(currentArtworkData.dominant_colors)) { // Compare arrays properly
          triggerMetadataUpdateBackend(activeArtworkId, { // UPDATED: Pass activeArtworkId
              ...
              keywords_from_image: currentArtworkData.keywords // Assuming keywords can also come from image analysis
          });
      }
    }
  ...
  const { data, isPending: isLoadingArtworkData } = useQuery({ // UPDATED: isLoading to isPending: isLoadingArtworkData
    ...
    queryFn: async () => {
      ...
      const [{ data: artworkData, error: artworkError }, { data: profileData, error: profileError }] = await Promise.all([
        supabase.from('artworks').select('*, artist:profiles!user_id(id, full_name, slug)').eq('id', artworkId).single(),
        supabase.from('profiles').select('id, full_name, default_has_certificate_of_authenticity').eq('id', user.id).single()
      ]);
      ...
      const { data: allUserCatalogues, error: allCatError } = await supabase
        .from('catalogues')
        .select('*, artist:profiles!user_id(id, full_name, slug)')
        .eq('user_id', user.id);
      ...
      return {
        artworkData: artworkData as AppArtwork,
        allUserCatalogues: (allUserCatalogues || []) as AppCatalogue[],
        assignedCatalogues,
        profileData: profileData as AppProfile | null,
        imgData: (imgData || []) as AppArtworkImage[]
      };
    },
  ...
  useEffect(() => {
    if (data?.artworkData) {
      ...
      // Ensure primaryArtworkImage is set correctly based on `is_primary` or the first image
  ...
  useEffect(() => {
    ...
    const currentArtistFullName = artistFullName || null;
    ...
    if (primaryArtworkImage && artworkId && artworkId !== 'new-artwork-temp-id' &&
      ...
      collectImageMetadataAndGenerateImages(primaryArtworkImage, artwork, currentArtistFullName || '', true);
    }
  ...
  const handleImageDragEnd = useCallback(async (event: DragEndEvent) => { // MARKED AS ASYNC
    ...
    setImages(prevImages => {
      ...
      if (artworkId && artworkId !== 'new-artwork-temp-id' && newPrimaryArtworkImage && newPrimaryArtworkImage.image_url !== artwork.primary_image_url) {
        ...
        setArtwork(prev => ({ ...prev, primary_image_url: newPrimaryArtworkImage.image_url }));
        setPrimaryArtworkImage(newPrimaryArtworkImage);
      }
      return finalOrder;
    });
  ...
  const handleDeleteImage = useCallback(async (id: string) => {
    ...
    // Corrected logic for primary image deletion constraint
    ...
    try {
      ...
      setImages(prevImages => {
        ...
        const newPrimaryArtworkImage = finalOrder.find(img => img.is_primary) || finalOrder[0] || null;
        if (newPrimaryArtworkImage?.image_url !== artwork.primary_image_url) {
          supabase.from('artworks')
            .update({ primary_image_url: newPrimaryArtworkImage?.image_url || null })
            .eq('id', artworkId)
            .then(({ error }) => {
              if (error) console.error("Error updating artwork's primary_image_url:", error.message);
            });
          setArtwork(prev => ({ ...prev, primary_image_url: newPrimaryArtworkImage?.image_url || null }));
          setPrimaryArtworkImage(newPrimaryArtworkImage);
        }
        return finalOrder;
      });
  ...
  const handleSetPrimary = useCallback(async (id: string) => { // MARKED AS ASYNC
    ...
    setImages(prevImages => {
      ...
      if (!newPrimaryCandidate || newPrimaryCandidate.is_primary) return prevImages;
      ...
      const primaryImgUrl = newOrder.find(img => img.is_primary)?.image_url || null;
      if (artworkId && artworkId !== 'new-artwork-temp-id' && primaryImgUrl !== artwork.primary_image_url) {
        supabase.from('artworks')
          .update({ primary_image_url: primaryImgUrl })
          .eq('id', artworkId)
          .then(({ error }) => {
            if (error) console.error("Error updating artwork's primary_image_url:", error.message);
          });
        setArtwork(prev => ({ ...prev, primary_image_url: primaryImgUrl }));
        setPrimaryArtworkImage(newOrder.find(img => img.is_primary) || null);
      }
      return newOrder;
    });
    setTouched((prev) => ({ ...prev, images: true }));
  ...
  const handleDropzoneImageAdd = useCallback(async (newImage: ArtworkImage) => {
    ...
    setImages(prev => {
        ...
        if (artworkId && artworkId !== 'new-artwork-temp-id' && finalOrder.length === 1 && newImage.image_url) {
            ...
            setPrimaryArtworkImage(newImage);
        }
        return finalOrder;
    });
  ...
  const handlePricingModelChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setArtwork((prev) => {
      ...
      if (newModel === 'fixed') {
        ...
        newArtwork.price = newArtwork.price ?? null; // Keep existing price if any
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      const artworkPayload: Database['public']['Tables']['artworks']['Insert'] = { // Use 'Insert' type for safe partial
        ...
        private_note: artwork.private_note || null,
        provenance_notes: artwork.provenance_notes || null,
        exhibitions: artwork.exhibitions || [],
        literature: artwork.literature || [],
        has_certificate_of_authenticity: artwork.has_certificate_of_authenticity ?? false,
        certificate_of_authenticity_details: artwork.has_certificate_of_authenticity ? (artwork.certificate_of_authenticity_details || null) : null,
        condition: artwork.condition || null,
        condition_notes: artwork.condition_notes || null,
        rarity: artwork.rarity || 'unique',
      ...
      let savedArtwork: Database['public']['Tables']['artworks']['Row'];
  ...
  return (
    <div className="artwork-form-layout"> {/* New wrapper for layout */}
        <ArtworkSidebar onSelectArtwork={(id) => onArtworkSelectedFromSidebar?.(id)} />

        <form id={formId} onSubmit={handleSubmit} className="artwork-form-main">
        ...
        <fieldset className="fieldset">
            ...
            {!!(artwork.signature_info?.is_signed) && (
                ...
                onChange={handleFormChange}
                onBlur={() => setTouched(prev => ({ ...prev.signature_info, location: true } ))}
