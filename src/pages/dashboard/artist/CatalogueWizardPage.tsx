...
const fetchExistingCatalogue = async (catalogueId: string) => {
    ...
    if (artworkIds.length > 0) {
        ...
        artworksInCatalogue = data || [];
    }

    return { catalogue, artworksInCatalogue };
};
...
const CatalogueWizardPage = () => {
    ...
    useEffect(() => {
        if (isEditing && existingCatalogueData) {
            ...
            setCoverArtworkId(coverArt?.id || (artworksInCatalogue.length > 0 ? artworksInCatalogue[0].id : null));
        }
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            ...
            if (!isSystem && !title.trim()) throw new Error("Title is required for user-created catalogues.");
        
            let finalCoverImageUrl = null;
            if (coverArtworkId) {
                const coverArt = [...selectedArtworks, ...availableArtworks].find(art => art.id === coverArtworkId);
        ...
        onSuccess: () => {
            ...
            alert(`Catalogue ${isEditing ? 'updated' : 'created'} successfully!`);
    ...
    const handleDelete = () => {
        if (window.confirm("Are you sure you want to permanently delete this catalogue? This cannot be undone.")) {
    ...
    return (
        <div style={{ padding: '2rem' }}>
            ...
            {isSystem && <p style={{ color: 'var(--muted-foreground)', marginTop: '-0.5rem' }}>You can manage the artworks and cover image for this system catalogue, but its title and description are fixed.</p>}
                                ...
                                {availableArtworks?.map(art => (
                                    <div key={art.id} className="item-list-row">
                                        ...
                                        <div style={{flexGrow: 1}}>
                                            <span>{art.title}</span>
                                            <p style={{fontSize: '0.75rem', color: 'var(--muted-foreground)', margin: 0, padding: 0}}>{art.dimensions ? `${(art.dimensions as any).height}x${(art.dimensions as any).width} ${(art.dimensions as any).unit || ''}` : ''} - {art.price ? formatPrice(art.price, art.currency) : 'N/A'}</p>
                                        </div>
