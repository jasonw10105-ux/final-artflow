...
const ArtworkWizardPage = () => {
    ...
    // Mutation for deleting an artwork from within the wizard
    const deleteMutation = useMutation({
        ...
        onSuccess: (removedId) => {
            ...
            // Optimistically update the query data to remove the item instantly from the UI
            ...
            // Invalidate the main artworks list in the background
    ...
    const handleTitleChange = (artworkIdToUpdate: string, newTitle: string) => {
        // Update the title in the sidebar optimistically
        queryClient.setQueryData(wizardQueryKey, (oldData: Artwork[] = []) => 
    ...
    const handleSaveAndNext = () => {
        ...
        // Invalidate the specific query for this form to refetch its data (including catalogue assignments)
        if (savedArtworkId) {
        ...
        if (currentIndex < artworkIds.length - 1) {
            ...
            // Invalidate the main list to show all new artworks
            queryClient.invalidateQueries({ queryKey: ['artworks'] });
            navigate('/artist/artworks');
        }
    };
    ...
    // This function tells the child form to submit itself
