...
export default function ArtworkListPage() {
  const [filters, setFilters] = useState<Filters>({ genre: [], status: [], keyword: [], search: "", sort: "newest" });
  ...
  const filtered = useMemo(() => {
    ...
    let list = artworks.filter((a) => {
      ...
      if (filters.status.length && !filters.status.includes(norm(a.status))) return false;
      if (filters.keyword.length && !a.keywords?.some(k => filters.keyword.includes(k))) return false;
      if (filters.search) {
        const q = filters.search.toLowerCase();
        const hitTitle = a.title?.toLowerCase().includes(q);
        const hitKw = a.keywords?.some(k => k.toLowerCase().includes(q));
        if (!hitTitle && !hitKw) return false;
      }
      return true;
    });

    switch (filters.sort) {
      case "price-low":  list = [...list].sort((a,b) => (a.price ?? 0) - (b.price ?? 0)); break;
      case "price-high": list = [...list].sort((a,b) => (b.price ?? 0) - (a.price ?? 0)); break;
      case "title-az":   list = [...list].sort((a,b) => (a.title ?? "").localeCompare(b.title ?? "")); break;
      case "title-za":   list = [...list].sort((a,b) => (b.title ?? "").localeCompare(a.title ?? "")); break;
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <h1>My Artworks</h1>
        <Link to="/artist/artworks/wizard">+ New Artwork</Link>
      </div>

      <div style={{ display:"flex", gap:16 }}>
        <FiltersSidebar artworks={artworks} filters={filters} setFilters={setFilters} />

        <div style={{ display:"grid", gap:16, gridTemplateColumns:"repeat(auto-fill, minmax(280px, 1fr))", flex:1 }}>
          {filtered.map((art) => {
            const cover = [...(art.artwork_images ?? [])].sort((a,b)=> (a.position ?? 0)-(b.position ?? 0))[0]?.image_url
              ?? "https://placehold.co/600x450?text=No+Image";

            const cat = art.artwork_catalogue_junction?.[0]?.catalogue;

            // public view by slug (if present)
            const viewHref = art.slug ? `/artwork/${art.slug}` : `/artist/artworks/edit/${art.id}`;
            ...
            return (
              <div key={art.id} style={{ border:"1px solid #e5e7eb", borderRadius:12, overflow:"hidden" }}>
                ...
                <div style={{ padding:12 }}>
                  ...
                  {art.price != null && <p>{(art.currency ?? "ZAR")} {art.price.toLocaleString()}</p>}
                  {art.genre && <p>Genre: {art.genre}</p>}
                  {art.keywords?.length ? <p>Keywords: {art.keywords.join(", ")}</p> : null}
                  {cat && cat.slug && (
                    <p>
                      Part of Catalogue: <Link to={`/${cat.slug}`}>{cat.title}</Link>
                    </p>
                  )}
