...
const ArtistCard = ({ artist }: { artist: ArtistWithExtras }) => (
    <div className="artist-discovery-card">
        <Swiper className="artist-card-swiper" spaceBetween={0} slidesPerView={1}>
            {artist.artwork_previews.map(art => (
                <SwiperSlide key={art.id}>
                    {/* Note: Ensure this link path is correct based on your App.tsx routing for artwork details */}
                    <Link to={`/artwork/${art.slug}`}>
                        <img
                            src={art.image_url}
                            alt="Artwork Preview"
                            className="artist-card-artwork-image"
                            loading="lazy" // Page Speed: Lazy load artwork previews in Swiper
                            // Consider srcset if your image service provides different sizes
                        />
                    </Link>
                </SwiperSlide>
            ))}
        ...
        <div className="artist-card-info">
            <Link to={`/u/${artist.slug}`} className="artist-card-avatar-link">
                <img
                    src={artist.avatar_url || '/placeholder.png'}
                    alt={artist.full_name || 'Artist'}
                    className="artist-card-avatar"
                    loading="lazy" // Page Speed: Lazy load artist avatars
                />
            </Link>
            <h3 className="artist-card-name"><Link to={`/u/${artist.slug}`}>{artist.full_name}</Link></h3>
...
const BrowseArtistsPage = () => {
    ...
    const pageTitle = "Discover Artists | ArtFlow App";
    const pageDescription = "Find your next favorite artist through curated and personalized selections on ArtFlow App.";
    const canonicalUrl = `${window.location.origin}/artists`;
    const ogImage = `${window.location.origin}/default-artists-share.jpg`; // A general image for this page

    if (isLoading) return (
      <>
        <SeoHelmet title={pageTitle} description={pageDescription} canonicalUrl={canonicalUrl} ogImage={ogImage} />
        <div className="page-container"><p className="loading-message">Loading artists...</p></div>
      </>
    );
    if (isError) return (
      <>
        <SeoHelmet title={`Error | ${pageTitle}`} description="Error loading artists." canonicalUrl={canonicalUrl} ogImage={ogImage} />
        <div className="page-container"><p className="error-message">Error loading artists: {error.message}</p></div>
      </>
    );
